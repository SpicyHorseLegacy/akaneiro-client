Shader "WaterColor/Transparent_Interactive"
{
	Properties 
	{
...
_TransparentPowerExponet("_TransparentPowerExponet", Float) = 3
...
float _TransparentPowerExponet;
float _Opacity;
...
//float4 Multiply8=Pow0 * float4(_EdgeWidth);
...
float4 Pow1=pow(Fresnel0,float4(_TransparentPowerExponet));
float4 Multiply2=Pow1 * float4(_Opacity);
float4 Multiply1=float4( Sampled2D0.a) * Multiply2;
