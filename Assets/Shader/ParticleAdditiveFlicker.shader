Shader "Particles/AdditiveFlicker"
{
	Properties 
	{
...
_FlickerSpeed("_FlickerSpeed", Float) = 1
...
float4 Multiply9=float4(_FlickerSpeed) * _Time;
float4 UV_Pan1=float4((IN.uv_NoiseTex.xyxy).x + Multiply9.x,(IN.uv_NoiseTex.xyxy).y + Multiply9.x,(IN.uv_NoiseTex.xyxy).z,(IN.uv_NoiseTex.xyxy).w);
float4 Tex2D2=tex2D(_NoiseTex,UV_Pan1.xy);
float4 Multiply10=Tex2D1 * Tex2D2;
float4 Multiply4=Multiply6 * Multiply10;
