Shader "Particles/FireDistortion"
{
	Properties 
	{
_TintColor("_TintColor", Color) = (0.5820895,0.5820895,0.5820895,1)
...
float4 Multiply0=_Time * float4(_DistortionSpeed);
...
float4 Add1=(IN.uv_MainTex.xyxy) + Multiply1;
float4 Multiply7=_Time * float4(_Speed);
float4 UV_Pan1=float4(Add1.x,Add1.y + Multiply7.y,Add1.z,Add1.w);
float4 Tex2D2=tex2D(_MainTex,UV_Pan1.xy);
float4 Multiply4=Multiply6 * Tex2D2;
float4 Multiply2=IN.color * Multiply4;
float4 Splat1=IN.color.w;
float4 Splat2=Multiply6.w;
float4 Add0=(IN.uv_MaskTex.xyxy) + Multiply1;
float4 Tex2D1=tex2D(_MaskTex,Add0.xy);
float4 Multiply9=Tex2D2 * float4( Tex2D1.a);
float4 Multiply5=Splat2 * Multiply9;
