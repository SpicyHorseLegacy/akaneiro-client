Shader "WaterColor/Tint_2Channels_Pulsing"{
	Properties 
	{
...
_PulsingBrightness("_PulsingBrightness", Float) = 0.53
_PulsingTex("_PulsingTex", 2D) = "white" {}
_PulsingSpeed("_PulsingSpeed", Float) = 0.41
...
float4 Multiply6=Add1 * float4(_PulsingBrightness);// Splat0 before
float4 Multiply12=_Time * float4(_PulsingSpeed);
float4 UV_Pan0=float4((IN.uv_PulsingTex.xyxy).x,(IN.uv_PulsingTex.xyxy).y + Multiply12.y,(IN.uv_PulsingTex.xyxy).z,(IN.uv_PulsingTex.xyxy).w);
float4 Tex2D1=tex2D(_PulsingTex,UV_Pan0.xy);
float4 Multiply9=Multiply6 * Tex2D1;
float4 Multiply11=Multiply10 * Multiply9;
