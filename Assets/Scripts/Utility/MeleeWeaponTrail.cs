...
public class MeleeWeaponTrail : MonoBehaviour
{
   ...
   public GameObject _o;
    ...
    void Start()
    {
		...
		_o.renderer.castShadows = false;
		_o.renderer.receiveShadows = false;

        _trailMesh = new Mesh();
        _trailMesh.name = name + "TrailMesh";
        _o.GetComponent<MeshFilter>().mesh = _trailMesh;
        DontDestroyOnLoad(_o);
    }

    void OnDisable()
    {
		// GameObject.SetActiveRecursively(false)会调用这里，所以每次使用需要隐藏武器的技能就会调用这里，然后把Trail删除了。
        //Destroy(_o);
    }
    ...
    void Update()
    {
        ...
        if (pointsToUse.Count > 1)
        {
            ...
            for (int n = 0; n < pointsToUse.Count; ++n)
            {
                ...
                {
                    float colorTime = time * (_colors.Count - 1);
                    float min = Mathf.Floor(colorTime);
                    float max = Mathf.Clamp(Mathf.Ceil(colorTime), 1, _colors.Count - 1);
                    float lerp = Mathf.InverseLerp(min, max, colorTime);
                    if (min >= _colors.Count) min = _colors.Count - 1; if (min < 0) min = 0;
                    if (max >= _colors.Count) max = _colors.Count - 1; if (max < 0) max = 0;
