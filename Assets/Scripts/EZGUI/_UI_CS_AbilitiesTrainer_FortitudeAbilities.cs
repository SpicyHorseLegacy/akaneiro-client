...
public class _UI_CS_AbilitiesTrainer_FortitudeAbilities : _UI_CS_AbilitiesTrainer_Base {
	
	public static _UI_CS_AbilitiesTrainer_FortitudeAbilities Instance;
	...
	public void SortListPriceLowToHigh(){
		m_AAItemList.Sort(_UI_CS_AbilitiesTrainer.Instance.CompareObjPriceLowToHigh);
	}
		...
		for(int i = 0;i<2;i++){
			if(childCount > i){
				...
				if(-2 !=  m_AAItemList[m_AbilitiesTrainerAllCurrentIdx].m_skVal && -2 !=  m_AAItemList[m_AbilitiesTrainerAllCurrentIdx].m_fkVal ){
					...
					//item.transform.GetComponent<_UI_CS_AbilitiesTrainerRawItemCtrl>().item[i].m_ValText.Text = "Max";
...
//				}
				...
				if(m_AbilitiesTrainerAllCurrentIdx >=  m_AAItemList.Count){
					m_AbilitiesTrainerAllCurrentIdx = 0;
					//LogManager.Log_Debug("m_AbilitiesTrainerAllCurrentIdx >=  m_AAItemList.Count");
				}
