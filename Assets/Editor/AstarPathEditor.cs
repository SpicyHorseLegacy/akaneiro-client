...
public class AstarPathEditor : Editor {
    ...
    {
		AstarPath AStart = target as AstarPath;
        ...
        path[path.Length - 1] = path[path.Length - 1].Replace(".unity", ".navmesh.xml");

        string strNavMeshFile = string.Join("/", path);
        ...
        xmlWriter.NodeBegin("navmesh");
			xmlWriter.AddAttribute("scene", sceneName);
                ...
                xmlWriter.AddAttribute("index", y);
					...
					for (int x = 0; x < nodeWidth; x++)
					{
						for (int z = 0; z < nodeDepth; z++)
						{
							...
							//xmlWriter.NodeBegin("node");
								//xmlWriter.AddAttribute("X", node.pos.x);
								//xmlWriter.AddAttribute("Y", node.pos.y);
								//xmlWriter.AddAttribute("Z", node.pos.z);
								//xmlWriter.AddAttribute("wX", node.vectorPos.x);
								//xmlWriter.AddAttribute("wY", node.vectorPos.y);
								//xmlWriter.AddAttribute("wZ", node.vectorPos.z);
                                ////xmlWriter.AddAttribute("en", node.walkable);

								//for (int c = 0; c < node.connections.GetLength(0); c++)
								//{
								//	Connection con = node.connections[c];
                                //    if (con.endNode != null && con.enabled)
								//	{
								//		xmlWriter.NodeBegin("con");
								//			xmlWriter.AddAttribute("X", con.endNode.pos.x);
								//			xmlWriter.AddAttribute("Y", con.endNode.pos.y);
								//			xmlWriter.AddAttribute("Z", con.endNode.pos.z);
								//			//xmlWriter.AddAttribute("angle", con.angle);
								//			xmlWriter.AddAttribute("cost", con.cost);
                                //            //xmlWriter.AddAttribute("en", con.enabled);
								//		xmlWriter.NodeEnd("con");
								//	}
								//}
							//xmlWriter.NodeEnd("node");
						}
					}
