...
public class BuildAtlases : ScriptableWizard
{
	...
	public static Material targetMaterial;	// Reference to the single material for which a single-material build will occur.
	...
	void OnWizardCreate()
	{
		...
		// If we're only building for the selected material,
		// remove all but the desired material:
		if (doSingleMaterial)
		{
			// Unset our flag:
			doSingleMaterial = false;

			MaterialSpriteList matList = null;
			for (int i = 0; i < matSpriteMap.Count; ++i)
				if (matSpriteMap[i].material == targetMaterial)
				{
					matList = (MaterialSpriteList)matSpriteMap[i];
					break;
				}

			// Remove all, then re-add just the one we want:
			matSpriteMap.Clear();

			if (matList != null)
				matSpriteMap.Add(matList);
			else
			{
				EditorUtility.DisplayDialog("No packable sprites found.", "No packable sprites found for material \"" + targetMaterial.name + "\".\n\nTry changing the \"Scan Project Folder\" setting.", "Ok");
				return;
			}
		}
