...
public class UICamera : MonoBehaviour
{
	...
	public delegate void Handle_selObjChanged(GameObject obj);
    public static event Handle_selObjChanged OnSelObjChanged;
	#endregion
	// Selected widget (for input)
	//20130710 changed.chenxinyi.//
	static GameObject mSelect = null;
	static GameObject mSel
	{
		get
		{
			return mSelect;
		...
		{
			mSelect = value;
			if(OnSelObjChanged != null) {
				OnSelObjChanged(mSelect);
			}
	...
	static public GameObject selectedObject
	{
		get
		{
//			GUILogManager.LogWarn("get: "+mSel.ToString());
			return mSel;
		}
		set
		{
	...
	void ProcessMouse ()
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			ProcessTouch(pressed, unpressed, i);
		}
		currentTouch = null;
	...
	void ProcessTouches ()
	{
		for (int i = 0; i < Input.touchCount; ++i)
		{
			...
			if (allowMultiTouch || input.fingerId == 0)
			{
				...
				ProcessTouch(pressed, unpressed, 0);
	...
	void ProcessOthers ()
	{
		...
		if (submitKeyDown || submitKeyUp)
		{
			...
			ProcessTouch(submitKeyDown, submitKeyUp, 0);
		}
	...
	void ProcessTouch (bool pressed, bool unpressed, int mouseKey)
	{
			...
			if (currentTouch.pressed != null)
			{
					...
					(currentTouch.clickNotification != ClickNotification.None && currentTouch.totalDelta.magnitude < drag))
				{
					...
					if (currentTouch.clickNotification != ClickNotification.None)
					{
                        ...
                        if (mouseKey == 0)
                            Notify(currentTouch.pressed, "OnLeftKeyClick", null);
                        if(mouseKey == 1)
                            Notify(currentTouch.pressed, "OnRightKeyClick", null);
		...
		if (currentTouch.pressed != null) {
			Notify(currentTouch.pressed, "OnDragOver", true);
		}
	}
