...
public class UIButtonTween : MonoBehaviour
{
	...
	void OnHover (bool isOver)
	{
		if (enabled)
		{
			if (trigger == NGUITrigger.OnHover ||
				(trigger == NGUITrigger.OnHoverTrue && isOver) ||
				(trigger == NGUITrigger.OnHoverFalse && !isOver))
			{
				Play(isOver);
			}
			mHighlighted = isOver;
	...
	void OnPress (bool isPressed)
	{
		if (enabled)
		{
			if (trigger == NGUITrigger.OnPress ||
				(trigger == NGUITrigger.OnPressTrue && isPressed) ||
				(trigger == NGUITrigger.OnPressFalse && !isPressed))
			{
				Play(isPressed);
			}
		}
	...
	void OnDoubleClick ()
	{
		if (enabled && trigger == NGUITrigger.OnDoubleClick)
		{
			Play(true);
		}
	}
	...
	void OnSelect (bool isSelected)
	{
		if (enabled)
		{
			if (trigger == NGUITrigger.OnSelect ||
				(trigger == NGUITrigger.OnSelectTrue && isSelected) ||
				(trigger == NGUITrigger.OnSelectFalse && !isSelected))
			{
				Play(true);
			}
		}
	...
	void OnActivate (bool isActive)
	{
		if (enabled)
		{
			if (trigger == NGUITrigger.OnActivate ||
				(trigger == NGUITrigger.OnActivateTrue && isActive) ||
				(trigger == NGUITrigger.OnActivateFalse && !isActive))
			{
				Play(isActive);
			}
		}
