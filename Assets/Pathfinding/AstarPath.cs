...
public class AstarPath : MonoBehaviour {
		...
		for (int y=0;y<grids.Length;y++) {//Height
			...
			foreach (Node node in staticNodes[y]) {
				...
				if (!node.walkable) {
					...
					Gizmos.DrawWireCube (node.vectorPos,Vector3.one * grid.nodeSize* 0.3f);
					//Gizmos.(node.vectorPos,grid.nodeSize*0.3F);
					continue;
    ...
    public static Vector3 GetNeasetPos(Vector3 _pos)
    {
        Int3 _posInt3 = ToLocal(_pos, true);
        if (_posInt3 == new Int3(-1, -1, -1)) return _pos;
        Node _node = AstarPath.GetNode(_posInt3);
        _node = AstarPath.GetNearest(_node, true, 240);
        return _node.vectorPos;
    }
